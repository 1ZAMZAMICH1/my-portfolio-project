// Файл: src/components/gallery-section.tsx

import React from "react";
import { motion } from "framer-motion";
import { Button, Spinner } from "@heroui/react";
import { Link } from "react-router-dom";
import { Icon } from "@iconify/react";
import { getGalleryImages } from "../services/gallery-service"; 
import { GalleryImage } from "../types/gallery"; 
import { getOptimizedUrl } from '../utils/image-optimizer'; // <-- ДОБАВЛЕН ИМПОРТ

const GallerySection: React.FC = () => {
  const [allImages, setAllImages] = React.useState<GalleryImage[]>([]);
  const [loading, setLoading] = React.useState(true);
  
  const col1Ref = React.useRef<HTMLDivElement>(null);
  const col2Ref = React.useRef<HTMLDivElement>(null);
  const col3Ref = React.useRef<HTMLDivElement>(null);
  const [colHeights, setColHeights] = React.useState({ col1: 0, col2: 0, col3: 0 });

  React.useEffect(() => {
    const loadImages = async () => {
      setLoading(true);
      try {
        const images = await getGalleryImages();
        setAllImages(images); 
      } catch (error) {
        console.error("Ошибка при загрузке изображений галереи на главной странице:", error);
      } finally {
        setLoading(false);
      }
    };
    loadImages();
  }, []);

  React.useEffect(() => {
    if (allImages.length > 0 && col1Ref.current && col2Ref.current && col3Ref.current) {
      const timer = setTimeout(() => {
        const heights = {
          col1: col1Ref.current?.scrollHeight || 0,
          col2: col2Ref.current?.scrollHeight || 0,
          col3: col3Ref.current?.scrollHeight || 0,
        };
        setColHeights(heights);
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [allImages]); 

  const chunkImagesIntoColumns = (images: GalleryImage[]) => {
    const cols: GalleryImage[][] = [[], [], []];
    images.forEach((image, index) => cols[index % 3].push(image));
    return cols;
  };

  const [column1, column2, column3] = React.useMemo(() => chunkImagesIntoColumns(allImages), [allImages]);

  const scrollAnimationVariants = (height: number, reverse: boolean) => {
    if (height === 0) return {};
    const scrollDistance = height / 5; 
    return {
      initial: { y: 0 },
      animate: {
        y: reverse ? [0, scrollDistance] : [0, -scrollDistance], 
        transition: { y: { duration: 20, ease: "linear", repeat: Infinity, repeatType: "loop" } },
      },
    };
  };

  const renderColumn = (column: GalleryImage[], ref: React.RefObject<HTMLDivElement>, colKey: string, height: number, reverse: boolean) => (
    <div className="overflow-hidden h-[600px] flex flex-col">
      <motion.div 
        ref={ref} 
        variants={scrollAnimationVariants(height, reverse)}
        animate="animate"
        className="flex flex-col gap-4"
      >
        {[...column, ...column, ...column, ...column, ...column].map((image, idx) => ( 
          <div key={`${colKey}-${image.id}-${idx}`} className="relative overflow-hidden group rounded-lg">
            <img 
              src={getOptimizedUrl(image.imageUrl, { width: 600 })} // <-- ИСПОЛЬЗУЕМ ФУНКЦИЮ С УКАЗАНИЕМ ШИРИНЫ
              alt={image.title} 
              className="w-full h-auto block"
              loading="lazy" // <-- ДОБАВЛЯЕМ ЛЕНИВУЮ ЗАГРУЗКУ
            />
          </div>
        ))}
      </motion.div>
    </div>
  );

  return (
    <section id="gallery" className="py-20 relative overflow-hidden">
      <div className="container mx-auto px-4">
        {/* ... твой заголовок ... */}
        <motion.h2 /* ... */ >
            <h2 className="text-4xl md:text-5xl font-bold mb-4">
                <span className="text-gradient">МОЯ</span> <span className="text-white">ГАЛЕРЕЯ</span>
            </h2>
            <div className="h-1 w-24 bg-red-500 mt-4"></div>
        </motion.h2>

        {loading ? (
          <div className="flex justify-center items-center py-20"><Spinner size="lg" /></div>
        ) : allImages.length === 0 ? (
          <div className="text-center py-20 text-foreground/70">
            <Icon icon="lucide:image" className="w-16 h-16 mx-auto mb-6 opacity-50" />
            <p className="text-lg">Галерея пока пуста.</p>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-4 max-h-[600px] overflow-hidden">
            {renderColumn(column1, col1Ref, "col1", colHeights.col1, false)}
            {renderColumn(column2, col2Ref, "col2", true, false)}
            {renderColumn(column3, col3Ref, "col3", colHeights.col3, false)}
          </div>
        )}
        
        {/* ... твоя кнопка ... */}
      </div>
    </section>
  );
};

export default GallerySection;